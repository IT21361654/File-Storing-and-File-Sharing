...
//Same site Cookie Vulnerability
// Middleware to parse cookies
app.use(cookieParser());

// Middleware to set SameSite attribute globally
...
http.listen(3000, function () {
    ...
    // get all files shared with logged-in user (Changed to fix the SQL injection Vulnerability)
    app.get("/SharedWithMe/:_id?", async function (request, result) {
    if (request.session.user) {
        try {
            // Use parameterized query to prevent SQL injection
            const userId = ObjectId(request.session.user._id);
            const files = await database.collection("shared_files").find({
                "userId": userId
            }).toArray();
                ...
                if (request.files.file.size > 0) {
                    ...
                    fileSystem.readFile(request.files.file.path, function (err, data) {
                        ...
                        fileSystem.writeFile(filePath, data, async function (err) {
                            ...
                            request.session.status = "success";
                            request.session.message = "The File has been succesfully uploaded";
